...
public class MyHashtable implements Iterable {
	...
	private void rehash() {
		...
		LinkedList[] tempTeble = new LinkedList[tableSize];
		for(int i=0; i<table.length; i++) {
			
			int hashcode = table[i].hashCode();
			int hash = hash(hashcode);
			
			Entry entry = new Entry(hash, table[i]);
			if(tempTeble[i]==null) {
				tempTeble[i] = new LinkedList();
			}
			tempTeble[i].add(entry);	
		}
		table = tempTeble;
	}
